...
export function ClusterView(props: ClusterViewProps) {
  ...
  return <section className="cluster-view">
    <header className="active-card-header"><div><p className="eyebrow">{props.referenceLabel ?? props.cluster.shortTitle}</p><h2>{props.referenceLabel ?? props.cluster.title}</h2><p>{props.cluster.purpose}</p><p className="progress-text">{completeCount}/{allTasks.length} tasks done</p></div>{props.onBackToOverview && <button type="button" className="back-button" onClick={props.onBackToOverview}><span aria-hidden="true">←</span> Back to Guided Practice overview</button>}</header>
